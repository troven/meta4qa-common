...
//		console.log("VAR: %j", this);
    ...
    learn.then(["$path in $varname should contain $match", "$path in $varname contains $match", "$path in $varname must contain $match"], function (path, name, match, done) {
    ...
    learn.then(["$path in $varname should be empty"], function (path, name, match, done) {
        var value = vars.findNamed(this, name);
        assert(value, "Variable " + name + " does not exist");
        var found = vars.findInPath(value, path);
